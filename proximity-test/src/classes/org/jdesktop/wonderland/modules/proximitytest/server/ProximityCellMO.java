...
public class ProximityCellMO extends CellMO {
            ...
            implements ProximityListenerSrv, ManagedObject, Serializable
    {
                                  ...
                                  int proximityIndex)
        {
           ...
           //CellManager cm = CellManager.getCellManager();
            //CellManager cm;
            CellMO proximityCellMO = CellManagerMO.getCell(cell);
            String proximityCellName = proximityCellMO.getName();
            CellMO avatarCellMO = CellManagerMO.getCell(viewCellID);
            String user = null;
            if (avatarCellMO instanceof AvatarCellMO) {
            user = ((AvatarCellMO)avatarCellMO).getUser().getUsername();
           logger.warning("****************************User: " + user); //This is the user that has just entered the bounds
          
           if(enter == "Enterred"){
           channelRef.get().sendAll(null,
                    new ProximityEnterExitMessage(proximityIndex, entered, user, proximityCellName));
           }
             } else {
                logger.warning("view cell is not an avatar: " + avatarCellMO);
                 return;
              }
